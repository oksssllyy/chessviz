...
int white_figure() {
  ...
  switch (point.board[x1][y1]) {
    ...
    case 'R':
      ...
      printf("Не, дружок, однако так так ладья не ходит :(.\n")
      break;
    case 'N':
    ...
    case 'B':
      ...
      if (((x1 % 2 == 1) && (y1 % 2 == 1)) || ((x1 % 2 == 0) && (y1 % 2 == 0))) {
        if (((x1 == 0 && y1 == 0) || (x1 == 7 && y1 == 7)) && (x2 == y2)) {
          return 1;
        } //по главной диагонали, при условии, что ход начинается из угла
        if ((x2 + y2) == (x1 + y1) || (((x2 - x1) % 2 == 0) && ((y2 - y1) % 2 == 0)) || 
            (((x2 - x1) % 2 == 1) && ((y2 - y1) % 2 == 1))) {
          return 1;
        } //остальные ходы
      }
